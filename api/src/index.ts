...
import {
  babies, caregivers, feedings, sleeps, pumpingSessions,
  diaperChanges, baths, growthEntries, doctorVisits,
} from './db/schema';
...
app.get('/api/babies/:babyId/caregivers', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const rows = await db.select().from(caregivers).where(eq(caregivers.babyId, c.req.param('babyId')));
  return c.json(rows);
});
...
app.delete('/api/babies/:babyId/caregivers/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  await db.delete(caregivers).where(and(
...
app.get('/api/babies/:babyId/feedings', async (c) => {
  ...
  const rows = await db.select().from(feedings)
...
app.put('/api/babies/:babyId/feedings/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const body = await c.req.json();
  await db.update(feedings).set(body).where(and(
...
app.delete('/api/babies/:babyId/feedings/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  await db.delete(feedings).where(and(
    eq(feedings.id, c.req.param('id')),
    eq(feedings.babyId, c.req.param('babyId')),
  ));
  return c.json({ ok: true });
...
app.get('/api/babies/:babyId/sleeps', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const rows = await db.select().from(sleeps)
...
app.put('/api/babies/:babyId/sleeps/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const body = await c.req.json();
  await db.update(sleeps).set(body).where(and(
...
app.delete('/api/babies/:babyId/sleeps/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  await db.delete(sleeps).where(and(
    eq(sleeps.id, c.req.param('id')),
    eq(sleeps.babyId, c.req.param('babyId')),
  ));
  return c.json({ ok: true });
...
app.get('/api/babies/:babyId/pumping', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const rows = await db.select().from(pumpingSessions)
...
app.post('/api/babies/:babyId/pumping', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const body = await c.req.json();
  const total = (body.leftMl ?? 0) + (body.rightMl ?? 0);
  const row = { id: uid(), babyId: c.req.param('babyId'), ...body, totalMl: total, createdAt: now() };
...
app.put('/api/babies/:babyId/pumping/:id', async (c) => {
  ...
  if (body.leftMl !== undefined || body.rightMl !== undefined) {
    const [existing] = await db.select().from(pumpingSessions).where(eq(pumpingSessions.id, c.req.param('id')));
    const left = body.leftMl ?? existing?.leftMl ?? 0;
    const right = body.rightMl ?? existing?.rightMl ?? 0;
    body.totalMl = left + right;
  }
  await db.update(pumpingSessions).set(body).where(and(
    eq(pumpingSessions.id, c.req.param('id')),
    eq(pumpingSessions.babyId, c.req.param('babyId')),
  ));
  const [row] = await db.select().from(pumpingSessions).where(eq(pumpingSessions.id, c.req.param('id')));
  return c.json(row);
});

app.delete('/api/babies/:babyId/pumping/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  await db.delete(pumpingSessions).where(and(
    eq(pumpingSessions.id, c.req.param('id')),
    eq(pumpingSessions.babyId, c.req.param('babyId')),
  ));
  return c.json({ ok: true });
...
app.get('/api/babies/:babyId/diapers', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const rows = await db.select().from(diaperChanges)
...
app.put('/api/babies/:babyId/diapers/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const body = await c.req.json();
  await db.update(diaperChanges).set(body).where(and(
    eq(diaperChanges.id, c.req.param('id')),
    eq(diaperChanges.babyId, c.req.param('babyId')),
  ));
  const [row] = await db.select().from(diaperChanges).where(eq(diaperChanges.id, c.req.param('id')));
  return c.json(row);
});

app.delete('/api/babies/:babyId/diapers/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  await db.delete(diaperChanges).where(and(
    eq(diaperChanges.id, c.req.param('id')),
    eq(diaperChanges.babyId, c.req.param('babyId')),
  ));
  return c.json({ ok: true });
...
app.get('/api/babies/:babyId/baths', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const rows = await db.select().from(baths)
...
app.put('/api/babies/:babyId/baths/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const body = await c.req.json();
  await db.update(baths).set(body).where(and(
...
app.delete('/api/babies/:babyId/baths/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  await db.delete(baths).where(and(
    eq(baths.id, c.req.param('id')),
    eq(baths.babyId, c.req.param('babyId')),
  ));
  return c.json({ ok: true });
...
app.get('/api/babies/:babyId/growth', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const rows = await db.select().from(growthEntries)
...
app.put('/api/babies/:babyId/growth/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const body = await c.req.json();
  await db.update(growthEntries).set(body).where(and(
    eq(growthEntries.id, c.req.param('id')),
    eq(growthEntries.babyId, c.req.param('babyId')),
  ));
  const [row] = await db.select().from(growthEntries).where(eq(growthEntries.id, c.req.param('id')));
  return c.json(row);
});

app.delete('/api/babies/:babyId/growth/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  await db.delete(growthEntries).where(and(
    eq(growthEntries.id, c.req.param('id')),
    eq(growthEntries.babyId, c.req.param('babyId')),
  ));
  return c.json({ ok: true });
...
app.get('/api/babies/:babyId/visits', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const rows = await db.select().from(doctorVisits)
...
app.put('/api/babies/:babyId/visits/:id', async (c) => {
  const db = drizzle(c.env.DB, { schema });
  const body = await c.req.json();
  await db.update(doctorVisits).set(body).where(and(
    eq(doctorVisits.id, c.req.param('id')),
    eq(doctorVisits.babyId, c.req.param('babyId')),
  ));
  const [row] = await db.select().from(doctorVisits).where(eq(doctorVisits.id, c.req.param('id')));
  return c.json(row);
});

// ─── Timeline ──────────────────────────────────────────────────────
// Returns all events for a day sorted newest-first with category tags
...
app.get('/api/babies/:babyId/stats/weekly', async (c) => {
  ...
  const avgFeedsPerDay = feedRows.length / 7;
  const totalPumpedMl = pumpRows.reduce((s, r) => s + (r.totalMl ?? 0), 0);
  const avgWetDiapersPerDay = diaperRows.filter(d => d.type === 'wet' || d.type === 'mixed').length / 7;
  const longestSleepSeconds = Math.max(0, ...sleepRows.map(s => s.durationSeconds ?? 0));

  return c.json({
    avgFeedsPerDay: Math.round(avgFeedsPerDay * 10) / 10,
    pumpingSessions: pumpRows.length,
    totalPumpedMl,
    avgWetDiapersPerDay: Math.round(avgWetDiapersPerDay * 10) / 10,
    longestSleepSeconds,
...
// ─── Seed ──────────────────────────────────────────────────────────
// POST /api/seed — idempotent first-run setup
...
app.post('/api/seed', async (c) => {
  ...
  await db.insert(doctorVisits).values({
    id: uid(), babyId,
    doctorName: 'Dr. Tan', hospital: 'KK Hospital',
