...
export function SleepScreen() {
  ...
  async function wakeUp() {
    ...
    setRunning(false);
    if (sessionId) {
      await (api.sleeps.update(sessionId, { endedAt: new Date().toISOString(), durationSeconds: elapsed, notes: notes || null }) as Promise<unknown>).catch(() => {});
    }
    setSaving(false);
    back();
  }
  ...
  async function saveManual() {
    ...
    await (api.sleeps.create({ location, notes: notes || null, loggedBy: 'You', startedAt: start.toISOString(), endedAt: end.toISOString(), durationSeconds: durSec }) as Promise<unknown>).catch(() => {});
    setSaving(false);
    back();
  }
  ...
  return (
    <div style={{ width: '100%', minHeight: '100%', background: '#2A3327', fontFamily: fonts.sans, display: 'flex', flexDirection: 'column', paddingTop: 'max(20px, env(safe-area-inset-top))', boxSizing: 'border-box', color: '#EDE7D6', position: 'relative' }}>
      ...
      <div style={{ padding: '6px 20px 0', display: 'flex', alignItems: 'center', gap: 10, position: 'relative' }}>
        <BackBtn dark />
        <div style={{ flex: 1, textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#EDE7D6', letterSpacing: 0.4, textTransform: 'uppercase' }}>Sleep</div>
      ...
      {entryMode === 'live' ? (
        <>
          <div style={{ padding: '30px 16px 0', textAlign: 'center', position: 'relative' }}>
            ...
            {running && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 16, padding: '5px 12px', borderRadius: 999, background: 'rgba(126,149,117,0.18)', fontSize: 11.5, color: T.sage, fontWeight: 600 }}>
                <span style={{ width: 6, height: 6, borderRadius: 3, background: T.sage }} /> {location}
              </div>
            )}
          </div>
        ...
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 12, position: 'relative' }}>
          ...
          {manualDurMin > 0 && (
            <div style={{ textAlign: 'center', fontFamily: fonts.serif, fontSize: 18, color: T.sage, fontStyle: 'italic' }}>
              {manualDurMin >= 60 ? `${Math.floor(manualDurMin / 60)}h ${manualDurMin % 60}m` : `${manualDurMin} min`}
            </div>
          )}
        </div>
      )}
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {locations.map(loc => (
            <button key={loc} onClick={() => setLocation(loc)} style={{
              padding: '7px 14px', borderRadius: 999, cursor: 'pointer', border: 'none',
              background: location === loc ? T.sage : 'rgba(255,252,245,0.08)',
              color: location === loc ? '#1F2A1D' : 'rgba(237,231,214,0.7)',
              fontFamily: fonts.sans, fontSize: 12, fontWeight: 600, textTransform: 'capitalize', transition: 'all 0.12s',
            }}>{loc}</button>
          ))}
        </div>
      </div>
