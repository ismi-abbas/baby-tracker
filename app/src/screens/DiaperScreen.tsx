...
export function DiaperScreen() {
  ...
  async function save() {
    ...
    await (api.diapers.create({ type, consistency: consistency ?? null, color: type !== 'wet' ? color : null, notes: notes || null, loggedBy: 'You', changedAt }) as Promise<unknown>).catch(() => {});
    setSaving(false);
    back();
  }
  ...
  return (
    <div style={{ width: '100%', minHeight: '100%', background: T.cream, fontFamily: fonts.sans, display: 'flex', flexDirection: 'column', paddingTop: 'max(20px, env(safe-area-inset-top))', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
          {types.map(t => (
            <div key={t.id} onClick={() => setType(t.id)} style={{
              ...
              background: type === t.id ? t.color : T.card, color: type === t.id ? T.card : T.ink,
              border: type === t.id ? 'none' : `1px solid ${T.rule}`,
              boxShadow: type === t.id ? `0 6px 18px ${t.color}55` : 'none', transition: 'all 0.15s',
            }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {(['Soft', 'Seedy', 'Watery', 'Hard', 'Mucousy'] as Consistency[]).map(c => (
                <div key={c} onClick={() => setConsistency(consistency === c ? null : c)} style={{
                  ...
                  background: consistency === c ? T.earth : 'transparent', color: consistency === c ? T.card : T.inkSoft,
                  border: consistency === c ? 'none' : `1px solid ${T.rule}`, transition: 'all 0.15s',
                }}>{c}</div>
              ))}
            </div>
            <div style={{ display: 'flex', gap: 10, marginTop: 12, alignItems: 'center' }}>
              ...
              {COLORS.map(c => (
                <div key={c} onClick={() => setColor(c)} style={{
                  ...
                  boxShadow: `0 0 0 1px ${T.rule}`, transform: color === c ? 'scale(1.2)' : 'scale(1)', transition: 'transform 0.1s',
                }} />
              ))}
            </div>
          </Card>
        ...
        <Card pad={14}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: useCustomTime ? 14 : 0 }}>
            <div>
              ...
              {!useCustomTime && (
                <div style={{ fontFamily: fonts.serif, fontSize: 20, color: T.ink, marginTop: 2 }}>
                  Now · {new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}
                </div>
              )}
            </div>
            <button onClick={() => { setUseCustomTime(v => !v); if (!useCustomTime) setCustomTime(toLocalDT(new Date())); }} style={{
              padding: '7px 14px', borderRadius: 12, border: `1.5px solid ${useCustomTime ? T.terracotta : T.rule}`,
              background: useCustomTime ? T.terracottaSoft : 'transparent',
              color: useCustomTime ? T.terracotta : T.inkSoft,
              fontFamily: fonts.sans, fontSize: 12.5, fontWeight: 700, cursor: 'pointer',
            }}>
              {useCustomTime ? '✓ Custom' : '✎ Backdate'}
            </button>
          </div>
          {useCustomTime && (
            <DateTimeField label="" value={customTime} onChange={setCustomTime} max={toLocalDT(new Date())} />
          )}
        </Card>
      </div>
      ...
      <div style={{ padding: '18px 16px 0', display: 'flex', gap: 10 }}>
        <button onClick={save} disabled={saving} style={{ ...primaryBtnStyle, flex: 1, background: T.earth }}>
