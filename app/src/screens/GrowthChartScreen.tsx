...
export function GrowthChartScreen() {
  ...
  useEffect(() => {
    (api.growth.list() as Promise<GrowthEntry[]>).then(e => setEntries(e.slice().reverse())).catch(() => {});
  }, []);

  const W = 320, H = 200;
  const wMin = 2, wMax = 12.5, xMin = 0, xMax = 12;

  const birthDate = new Date('2026-02-23');
  function ageMonths(iso: string) {
    const d = new Date(iso);
    return (d.getTime() - birthDate.getTime()) / (1000 * 3600 * 24 * 30.44);
  }
      ...
      <div style={{ padding: '12px 16px 0' }}>
        <div style={{ display: 'flex', padding: 4, borderRadius: 14, background: 'rgba(0,0,0,0.04)', gap: 2 }}>
          {(['Weight', 'Length', 'Head'] as Metric[]).map(m => (
            <div key={m} onClick={() => setMetric(m)} style={{
              flex: 1, padding: '8px 10px', borderRadius: 11, textAlign: 'center', fontSize: 12, fontWeight: 700, cursor: 'pointer',
              background: m === metric ? T.card : 'transparent',
              color: m === metric ? T.rose : T.inkSoft,
              boxShadow: m === metric ? '0 1px 2px rgba(0,0,0,0.04)' : 'none',
            }}>{m}</div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '14px 22px 0', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontFamily: fonts.serif, fontSize: 44, color: T.ink, fontWeight: 500, lineHeight: 1, letterSpacing: -1, marginTop: 2 }}>
            {weightKg.toFixed(1)}<span style={{ fontSize: 16, color: T.inkMute, fontStyle: 'italic', marginLeft: 4 }}>kg</span>
          </div>
          {delta !== null && (
        ...
        <Chip color={T.rose} soft={T.roseSoft} style={{ padding: '6px 12px', fontSize: 12.5 }}>~52nd percentile</Chip>
      </div>
      ...
      <div style={{ padding: '14px 16px 0' }}>
        <Card pad={14}>
          <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="180">
            ...
            {/* Percentile band */}
            <path
              d={`${path(WHO_P3)} L ${px(xMax)} ${py(WHO_P97[12])} ${[...WHO_P97].reverse().map((y, i) => `L ${px(WHO_MONTHS[12 - i])} ${py(y)}`).join(' ')} Z`}
              fill={T.roseSoft} opacity="0.45"/>
            ...
            {babyPoints.map(([x, y], i) => (
              <g key={i}>
                <circle cx={px(x)} cy={py(y)} r="5" fill={T.card} stroke={T.terracotta} strokeWidth="2"/>
              </g>
            ))}
            {babyPoints.length > 0 && (
              <>
                <circle cx={px(babyPoints[babyPoints.length-1][0])} cy={py(babyPoints[babyPoints.length-1][1])} r="9" fill="none" stroke={T.terracotta} strokeWidth="1" opacity="0.4"/>
                <g transform={`translate(${px(babyPoints[babyPoints.length-1][0]) + 8}, ${py(babyPoints[babyPoints.length-1][1]) - 18})`}>
                  <rect x="0" y="0" width="58" height="22" rx="6" fill={T.terracotta}/>
                  <text x="6" y="14" fontFamily="Hanken Grotesk" fontWeight="700" fontSize="10" fill={T.card}>
                    Saif · {weightKg.toFixed(1)}
                  </text>
                </g>
              </>
            )}
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8, fontSize: 10.5, color: T.inkMute, flexWrap: 'wrap' }}>
            <span><span style={{ display: 'inline-block', width: 10, height: 2, background: T.terracotta, verticalAlign: 'middle', marginRight: 4 }}/> Saif</span>
